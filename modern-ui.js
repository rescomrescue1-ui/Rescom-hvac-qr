/* Res-Com HVAC QR v30.10 — Reliable Temporary Passwords + Separate Staff QR Auth */
(() => {
  "use strict";

  const MODERN_VERSION = "30.10";
  const MODERN_BUILD = "Aug 19, 2026 5:40 PM";
  ...
  function updateManualAndPatchNotes(){
    ...
    const acctSec=byId("m-accounts");if(acctSec&&acctSec.dataset.rcManualTeam!==MODERN_VERSION){acctSec.innerHTML=`<h3>Team Accounts</h3><p>Res-Com now uses one simple Team account system. The first person who creates an account becomes Admin automatically.</p><p>Every Admin and Tech signs in with their own email and password. New employee self-signups require the current 4-digit company PIN, which only Admins manage.</p><p>Admins can create accounts, promote Techs to Admin, delete employee logins, reset temporary passwords, generate one-time Staff Sign-In QR codes, and rotate the company PIN from the Team tab. Signed-out employees can also use FORGOT / RESET PASSWORD with their account email, the current Company PIN, and a new password.</p>`;acctSec.dataset.rcManualTeam=MODERN_VERSION;}
